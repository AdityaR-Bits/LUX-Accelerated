...
#             if lux.config.render_widget:
#                 self._widget = rec_df.render_widget()
        #saved_data = pd.DataFrame(rec_infolist).to_csv("saved_interesting_plots_cudf.csv")
        print("rec_info here : ", rec_infolist)
        self._recs_fresh = True
#         s =time.time()
#         dat = pd.read_csv("saved_interesting_plots.csv") #in pandas as few rows only
#         df = self#cudf.read_csv("data/nyc_taxi.csv")
#         adder = False
#         count=0
