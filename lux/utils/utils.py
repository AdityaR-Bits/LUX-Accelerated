...
def check_if_id_like(df, attribute):
    ...
    is_string = isinstance(df[attribute],cudf.core.column.string.StringMethods) #pd.api.types.is_string_dtype(df[attribute])
    if is_string:
...
#                 sampled = SQLExecutor.execute_preview(df, preview_size=50)
        ...
        if len(df) >= 2:
            series = df[attribute]
            diff = series.diff()
            # if diff.iloc[1]*(diff.shape[0]-1)==diff.iloc[1:].sum():
            #     print("even soacle")
            # diff = cudf.DataFrame(diff.dropna())
            # print("different here", diff)
            # col = diff.columns[0]
            # df_compare = diff.reset_index()[col]
            # new_df = diff.loc[1].repeat(diff.shape[0])
