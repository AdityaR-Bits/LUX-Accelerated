...
def is_timedelta64_series(series: cudf.Series) -> bool:
    """
    ...
    return series.dtype=='timedelta[ns]'#isinstance(series, cudf.TimedeltaIndex)


def timedelta64_to_float_seconds(series: cudf.Series) -> cudf.Series:
    """
...
def date_formatter(time_stamp, ldf):
    ...
    datetime = cudf.to_datetime(time_stamp)
...
def compute_date_granularity(date_column: cudf.core.series.Series):
    """
    ...
    date_index = cudf.DatetimeIndex(date_column)
...
def is_datetime_series(series: cudf.Series) -> bool:
    ...
    is_date: bool
    """
    #pd.api.types.is_datetime64_any_dtype(series) or pd.api.types.is_period_dtype(series)
