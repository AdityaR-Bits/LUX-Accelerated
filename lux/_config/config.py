...
class Config:
    ...
    _plotting_backend = "vegalite"
    _plotting_scale = 1
    _topk = 15
    _number_of_bars = 10  # max no of bars displayed (rest shown as "+ k more")
    _label_len = 25  # max length of x and y axis labels
    _sort = "descending"
    _pandas_fallback = True
    _interestingness_fallback = True
    heatmap_bin_size = 40
    tracer_relevant_lines = []
    tracer = LuxTracer()
    query_templates = {}
    handle_quotes = True
    #####################################
    ...
    #####################################
    _sampling_start = 100000
    _sampling_cap = 1000000
    _sampling_flag = True
    _heatmap_flag = True
    _heatmap_start = 5000
    lazy_maintain = True
    early_pruning = True
    early_pruning_sample_cap = 30000
    # Apply sampling only if the dataset is 150% larger than the sample cap
    early_pruning_sample_start = early_pruning_sample_cap * 1.5
    streaming = False
    render_widget = True
